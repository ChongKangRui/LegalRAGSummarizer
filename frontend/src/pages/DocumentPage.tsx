...
export default function DocumentPage() {
  ...
  function handleCiteClick(citation: Citation) {
    const clauseId = citation.chunkId.split(":").slice(1).join(":")
  ...
  function handleExport() {
    if (!doc || !summarize.data) return
    const lines = [
      `# ${doc.title}`,
      "",
      `**Question:** ${summarize.data.query}`,
      "",
      summarize.data.answer,
      "",
      "## Citations",
      ...summarize.data.citations.map(
                  ...
                  <div className="mt-2 space-y-3 border-l pl-4">
                    {section.clauses.map((clause) => (
                      <div
                        ...
                        id={`clause-${clause.id}`}
                        className={cn(
                          "scroll-mt-4 rounded-md border border-transparent px-2 py-1.5 text-sm text-muted-foreground transition-colors",
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button onClick={handleAsk} disabled={!query.trim() || summarize.isPending}>
                {summarize.isPending ? (
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-1.5">
                <p className="text-xs font-medium text-muted-foreground">
                  Citations ({summarize.data.citations.length})
                </p>
                <ul className="space-y-1 text-xs">
                  {summarize.data.citations.map((c, i) => (
                    <li
                      key={i}
                      className={cn(
